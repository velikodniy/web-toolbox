...
test('URLEncoder encodes URL in real-time', async () => {
  ...
  fireEvent.change(decodedInput, {
    target: { value: 'https://example.com/foo bar' },
  });

  await waitFor(() => {
    expect((encodedInput as HTMLTextAreaElement).value).toBe(
      encodeURIComponent('https://example.com/foo bar'),
    );
  }, { timeout: 1000 });
  cleanup();
});
...
test('URLEncoder decodes URL in real-time', async () => {
  ...
  await waitFor(() => {
    expect((decodedInput as HTMLTextAreaElement).value).toBe(
